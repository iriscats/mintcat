...
pub fn integrate<P: AsRef<Path>>(
    ...
    for (mod_info, path) in &mods {
        ...
        if magic == [0x50, 0x4B, 0x03, 0x04] {
            ...
            pak_buf =
                get_pak_from_data(Box::new(BufReader::new(raw_mod_file)), "pak").map_err(|e| {
            ...
            let buf =
                get_pak_from_data(Box::new(BufReader::new(raw_mod_file)), "dll").map_err(|e| {
                    if let IntegrationError::IoError { source } = e {
                        IntegrationError::CtxtIoError {
                            source,
                            mod_info: mod_info.clone(),
        ...
        // is pak file
            ...
            pak_buf = Box::new(BufReader::new(raw_mod_file));
        }
