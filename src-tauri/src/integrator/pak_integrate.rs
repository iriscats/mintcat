...
static MOD_PAK_PATH: str = *"FSD-WindowsNoEditor_MintCat_Mods.pak";

static ASSET_REGISTRY_PATH: &str = "FSD/AssetRegistry.bin";


pub(crate) struct PakIntegrator {
...
impl PakIntegrator {
    fn new(game_pak_path: PathBuf, mods: Vec<(ModInfo, PathBuf)>) -> PakIntegrator {

        let mut fsd_pak_reader = BufReader::new(fs::File::open(game_pak_path.as_ref())?);
        let fsd_pak = repak::PakBuilder::new().reader(&mut fsd_pak_reader)?;

        let mut asset_registry = AssetRegistry::read(&mut Cursor::new(
            fsd_pak.get(ASSET_REGISTRY_PATH, &mut fsd_pak_reader)?,
        ...
        PakIntegrator {
            game_pak_path,
            mods,
            installation: DRGInstallation::default(),
            asset_registry,
        }
    }

    fn get_mod_pak_full_path(&mut self) -> Result<PathBuf, IntegrationError> {
        let Ok(installation) = DRGInstallation::from_pak_path(&self.game_pak_path) else {
            return Err(IntegrationError::DrgInstallationNotFound {
                path: self.game_pak_path.as_ref().to_path_buf(),
            });
        };

        self.installation = installation;
        info!("installation path {:?}", self.installation.paks_path());
        self.installation.paks_path().join(&MOD_PAK_PATH)
    }

    fn patch_game_asset(&mut self) {

    }


    fn collect_assets_from_mods(&mut self) {
    ...
    fn write_asset_registry(&mut self){

    }

    fn repack_mods(&mut self) {
...
pub(crate) fn integrate(
    ...
    let path_mod_pak = get_mod_pak_full_path(game_pak_path);
    let mut fsd_pak_reader = BufReader::new(fs::File::open(game_pak_path.as_ref())?);
    let fsd_pak = repak::PakBuilder::new().reader(&mut fsd_pak_reader)?;
    ...
    for patch_path in patch_paths {
        patch_deferred(patch_path, game_pak_patch::patch)?;
    }
    ...
    patch_deferred(
        ...
        game_pak_patch::patch_server_list_entry,
    )?;
    ...
    bundle.write_file(&buf, ASSET_REGISTRY_PATH)?;
    bundle.finish()?;

    info!(
        "{} mods installed to {}",
        mods.len(),
        game_pak_path.display()
    );
...
pub(crate) fn uninstall<P: AsRef<Path>>(
    path_pak: P,
    ...
    let path_mods_pak = installation.paks_path().join(&MOD_PAK_PATH);
