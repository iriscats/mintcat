...
pub fn write_asset<P: AsRef<Path>>(
    path: P,
    asset: Asset<Cursor<Vec<u8>>>,
) -> Result<unreal_asset::Asset<Cursor<Vec<u8>>>> {
    let mut uasset = Cursor::new(vec![]);
    let mut uexp = Cursor::new(vec![]);
    asset.write_data(&mut uasset, Some(&mut uexp))?;
    fs::write(&path, uasset.into_inner())?;
    fs::write(path.as_ref().with_extension("uexp"), uexp.into_inner())?;
    Ok(asset)
}
